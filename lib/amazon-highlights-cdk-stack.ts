  ...
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const getContentsLambda = new lambda.Function(this, "getContents", {
      code: new lambda.InlineCode(
        fs.readFileSync("lambda/getContents.py", { encoding: "utf-8" })
      ),
    ...
    const getScriptLambda = new lambda.Function(this, "getScript", {
      code: new lambda.InlineCode(
        fs.readFileSync("lambda/getScript.py", { encoding: "utf-8" })
      ),
      handler: "index.lambda_handler",
      timeout: cdk.Duration.seconds(30),
      runtime: lambda.Runtime.PYTHON_3_9,
    ...
    const getSummaryLambda = new lambda.Function(this, "getSummary", {
      code: new lambda.InlineCode(
        fs.readFileSync("lambda/getSummary.py", { encoding: "utf-8" })
      ),
      handler: "index.lambda_handler",
      timeout: cdk.Duration.seconds(30),
      runtime: lambda.Runtime.PYTHON_3_9,
    ...
    const putResultLambda = new lambda.Function(this, "putResult", {
      code: new lambda.InlineCode(
        fs.readFileSync("lambda/putResult.py", { encoding: "utf-8" })
      ),
      handler: "index.lambda_handler",
      timeout: cdk.Duration.seconds(30),
      runtime: lambda.Runtime.PYTHON_3_9,
      environment: {
        DYNAMODB_TABLE_NAME: contentsTable.tableName,
        REGION: contentsTable.env.region,
      },
    });

    const sendEmailLambda = new lambda.Function(this, "sendEmail", {
      code: new lambda.InlineCode(
        fs.readFileSync("lambda/sendEmail.py", { encoding: "utf-8" })
      ),
      handler: "index.lambda_handler",
      timeout: cdk.Duration.seconds(30),
    ...
    rule.addTarget(
      new targets.LambdaFunction(getContentsLambda, {
    ...
    rule.addTarget(
      new targets.LambdaFunction(sendEmailLambda, {
        maxEventAge: cdk.Duration.hours(2), // Optional: set the maxEventAge retry policy
        retryAttempts: 2, // Optional: set the max number of retry attempts
      })
    );
    ...
    const getScript = new tasks.LambdaInvoke(this, "getScriptLambda", {
      lambdaFunction: getScriptLambda,
      outputPath: "$.Payload",
    });
    const getSummary = new tasks.LambdaInvoke(this, "getSummaryLambda", {
      lambdaFunction: getSummaryLambda,
      outputPath: "$.Payload",
    });
    const putResult = new tasks.LambdaInvoke(this, "putResultLambda", {
      lambdaFunction: putResultLambda,
      outputPath: "$.Payload",
    });

    //create chain
    const fail = new sfn.Fail(this, "fail");
    const chain = sfn.Chain.start(getScript).next(getSummary).next(putResult);
    ...
    const startStateMachineLambda = new lambda.Function(this, "startLambda", {
      code: new lambda.InlineCode(
        fs.readFileSync("lambda/startStateMachine.py", { encoding: "utf-8" })
      ),
      handler: "index.lambda_handler",
      timeout: cdk.Duration.seconds(30),
      runtime: lambda.Runtime.PYTHON_3_9,
    ...
    contentsBucket.addEventNotification(
